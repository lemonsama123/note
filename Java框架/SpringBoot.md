...
## 33、请求处理-【源码分析】-Servlet API 参数解析原理
...
![在这里插入图片描述](https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/8919/20210205010509984.png)
...
可用 Postman 软件分别测试返回 json 和 xml：只需要改变请求头中 Accept 字段（application/json、application/xml）。
...
这样，后端会根据参数 format 的值，返回对应 json 或 xml 格式的数据。
...
这个设置后，URL 要插入`/app`, 如`http://localhost:8080/app/hello.html`。
